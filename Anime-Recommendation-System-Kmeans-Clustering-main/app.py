...
# Custom tokenizer to split genres based on commas, with handling for empty or missing genres
...
@app.route('/login')
def login():
    return render_template('http://localhost:3005')
...
def recommend_anime(input_title, cluster_column, df):
    ...
    if not result:
        return {'error': 'No match found for the title provided.'}
...
from flask import Flask, render_template, request, jsonify, redirect  # Import redirect here
import numpy as np
import pandas as pd
import pickle
import h5py
...
@app.route('/login')
def login():
    return redirect('http://localhost:3006/sign-in')  # Ensure redirect works with Flask
...
def find_similar_users(item_input, n=10, return_dist=False, neg=False):
    try:
        ...
        print(f'\033[1m{item_input}\033[0m, Not Found in User list')
...
def recommend():
    ...
    if recommendation_type == "user_based":
        ...
        if not user_id:
            return render_template('index6.html', error_message="Please enter a User ID.", recommendation_type=recommendation_type)
        try:
            user_id = int(user_id)
        except ValueError:
            return render_template('index6.html', error_message="Please enter a valid User ID (must be an integer).", recommendation_type=recommendation_type)
        ...
        if similar_user_ids is None or similar_user_ids.empty:
            ...
            if response.status_code == 200:
                ...
                if 'data' not in data:
                    ...
                    message1 = "No anime recommendations available for the given user."
            else:
                message1 = f"User with user_id {user_id} does not exist in the database."
        ...
        return render_template('index7.html', animes=recommended_animes, recommendation_type=recommendation_type)

    elif recommendation_type == "item_based":
        anime_name = request.form['anime_name']
        
        if not anime_name:
            return render_template('index6.html', error_message="Please enter Anime name.", recommendation_type=recommendation_type)
        ...
        if recommended_animes is None or recommended_animes.empty:
            message2 = f"Anime {anime_name} does not exist"
            return render_template('index7.html', message=message2, animes=None, recommendation_type=recommendation_type)
        
        return render_template('index7.html', animes=recommended_animes, recommendation_type=recommendation_type)

    else:
        return render_template('index6.html', error_message="Please select a recommendation type.")
